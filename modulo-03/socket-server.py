#Os argumentos passados socket() são constantes usadas para especicar a família de endereços é o tipo de soquete. AF_INET é a família de endereços da Internet para IPv4. SOCK_STREAM é o tipo de soquete para TCP,
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    #O .bind() método é usado para associar o soquete a uma interface de rede e número de porta especifico:
    s.bind((HOST,PORT))
    ...
    #O soque deve estar vinculado a um endereço e escutar conexões.
    conn, addr = s.accept()
