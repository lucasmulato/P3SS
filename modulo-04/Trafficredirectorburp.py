...
#Criando duas funções com os nomes das bibliotecas para implementa-las
class BurpExtender( IBurpExtender, IHttpListener):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        # definir o nome da  exteãnso
        callbacks.setExtensionName("Traffic redirector")

        # habilitar o modo Debuger para coletaar erros o stdout coleta entrada e saida de erros
        self.stdout = PrintWriter(callbacks.getStdout(). True):
        self.stdout.println("DEBUG: Habilitado!")

        # Crie um ouvinte HTTP
        callbacks.registerHttpListener(self)

    #
    # implementando IHttpListener
    #

    def processHttpMessage(self, toolFlag, messageIsRequet, messageInfo):
        # Ele mostra no debug o processamento das solicitações
        self.stdout.println("DEBUG: Entrando na função processHttpMessage")
        # apenas processar solicitações
        if not messageIsRequest:
            self.stdout.println("Debug: Essa mensagem não é uma requisição, parando processo...")
            return
        ...
        # se o host for HOST_FROM, mude para HOST_TO, ele vai usar iHTTPListener para fazer o redirecionamento, abaixo vamos configurar todo processo
        if (HOST_FROM == httpService.getHost()):
            self.stdout.prinln("Debug: HOST_TO and HOST_FROM são o mesmo servidor")
